...
_enemySide = side _source;

if( isNil "_enemySide")then{
_enemySide = east;
};
...
(!(_source == _unit)) && 
((side _unit) == west) &&
(((_source isKindOf "Car" || _source isKindOf "Motorcycle" || _source isKindOf "Ship" || _source isKindOf "Air" || _ammo == "CarHorn")) 
|| ( _enemySide != west))) then {
_allowed = false;
};
